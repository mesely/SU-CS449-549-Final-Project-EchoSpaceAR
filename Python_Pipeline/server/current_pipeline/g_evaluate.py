...
`labels` column after mapping labels into the semantic EchoSpace label set.
"""
...
from . import b_config as current_config
import legacy_pipeline.b_legacy_config as legacy_config
from .c_audio_core import (
    SpatialSnapshot,
    YamnetClassifier,
    downmix_to_mono,
    ensure_frame_major,
...
from .b_config import LOG_DIR, SEMANTIC_LABEL_SET, resample_linear, tf
...
def normalize_sonyc_label(raw_label: str) -> str:
    """Normalize SONYC / FSD style source labels into a stable lookup key."""
...
SONYC_LABEL_MAP = {
    normalize_sonyc_label(alias): semantic_label
    for semantic_label, aliases in SONYC_LABEL_ALIASES.items()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--restrict-to-manifest-labels",
        action="store_true",
        help="Only score semantic labels that appear in the current manifest.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--label-map-json",
        default=None,
        help="Optional JSON file that maps source labels into the semantic label space.",
    )
    parser.add_argument(
        "--summary-csv",
        default=SUMMARY_CSV,
    ...
    parser.add_argument(
        ...
        type=int,
        default=25,
        help="Balanced quota per mapped semantic label for SONYC scene selection.",
    )
    parser.add_argument(
        "--sonyc-seed",
        type=int,
...
def load_label_map(path: str | None) -> dict[str, str]:
    """Load an optional source-label to semantic-label mapping."""
    if not path:
        return {}
...
def read_wav_frames(path: str) -> tuple[np.ndarray, int]:
    """Read WAV audio without depending on TensorFlow or soundfile."""
    ...
    return ensure_frame_major(array), int(sample_rate)


def write_wav_frames(path: str, frames: np.ndarray, sample_rate: int) -> None:
    """Write float audio to WAV using scipy as a TensorFlow-free fallback."""
...
def parse_sonyc_scene(annotation_path: str, vocab: list[str]) -> SonicSceneSpec | None:
    ...
    for item in events:
        ...
        if semantic_label:
            mapped_labels.append(semantic_label)
...
def scene_sources_exist(scene: SonicSceneSpec, source_root: str) -> bool:
    ...
    return all(
        ...
        if event.semantic_label is not None
    )
...
def choose_sonyc_scenes(
    ...
    """Choose a balanced subset of SONYC scenes by semantic label."""
...
def render_sonyc_scene(
    ...
    def load_source_mono(source_rel_path: str) -> np.ndarray:
        ...
        if sf is not None:
            ...
            frames, input_sr = read_wav_frames(absolute_path)
    ...
    mapped_events = [event for event in scene.foreground_events if event.semantic_label is not None]
    ...
    write_wav_frames(output_path, mix.reshape((-1, 2 if stereo else 1)), SONYC_SAMPLE_RATE)
    ...
    with open(manifest_path, "r", encoding="utf-8", newline="") as handle:
        ...
        for row_index, row in enumerate(reader, start=1):
            ...
            for label in raw_labels:
                ...
                if mapped not in SEMANTIC_LABEL_SET:
                    print(
                        f"[WARN] Satir {row_index}: '{mapped}' semantic label setinde yok, atlandi.",
                        file=sys.stderr,
                    )
                    continue
...
def load_audio_frames(path: str) -> tuple[np.ndarray, int]:
    ...
    if sf is not None:
        ...
        frames, sample_rate = read_wav_frames(path)
    if frames.size == 0:
...
def infer_active_label_set(samples: list[SampleRecord]) -> set[str]:
    """Infer the semantic label subset that is actually evaluated in this manifest."""
    return {
        label
        for sample in samples
        for label in (sample.positive_labels or (sample.label,))
        if label not in NEUTRAL_OUTPUT_LABELS
    }


def mask_probabilities_to_label_set(
    labels: list[str],
    probabilities: np.ndarray,
    active_labels: set[str] | None,
) -> np.ndarray:
    """Zero out labels that are outside the manifest's semantic target space."""
    if not active_labels:
        return np.asarray(probabilities, dtype=np.float64)

    mask = np.array([label in active_labels for label in labels], dtype=bool)
    if not np.any(mask):
        return np.asarray(probabilities, dtype=np.float64)

    masked = np.asarray(probabilities, dtype=np.float64).copy()
    masked[~mask] = 0.0
    return masked
...
def _best_support_run(sequence: np.ndarray, threshold: float) -> tuple[int, float]:
    """Return the longest confident run and its mean score."""
    best_len = 0
    best_mean = 0.0
    start = None
    for index, score in enumerate(sequence):
        if float(score) >= threshold:
            if start is None:
                start = index
            continue
        if start is not None:
            segment = sequence[start:index]
            if segment.size > best_len:
                best_len = int(segment.size)
...
def _compute_temporal_clip_scores(
    labels: list[str],
    raw_prob_trace: list[np.ndarray],
    fallback_peak: np.ndarray,
) -> np.ndarray:
    """Collapse window probabilities into class-aware clip scores."""
    if not current_config.TEMPORAL_CLIP_AGGREGATION_ENABLED or not raw_prob_trace:
        return np.asarray(fallback_peak, dtype=np.float64)

    stacked = np.vstack([np.asarray(item, dtype=np.float64).reshape(1, -1) for item in raw_prob_trace])
    scores = np.asarray(fallback_peak, dtype=np.float64).copy()
    top_k = min(2, stacked.shape[0])
    for index, label in enumerate(labels):
        sequence = stacked[:, index]
        peak = float(np.max(sequence))
        strongest = np.partition(sequence, -top_k)[-top_k:]
        top_support = float(np.mean(strongest))
        threshold = max(float(current_config.TEMPORAL_CLIP_MIN_ABS_SUPPORT), peak * float(current_config.TEMPORAL_CLIP_MIN_REL_SUPPORT))
        run_len, run_mean = _best_support_run(sequence, threshold)
        run_factor = min(1.0, float(run_len) / 2.0)
        family = current_config.TEMPORAL_FAMILY_BY_LABEL.get(label)
        if family == "transient":
            score = (
                float(current_config.TEMPORAL_CLIP_TRANSIENT_PEAK_WEIGHT) * peak
                + float(current_config.TEMPORAL_CLIP_TRANSIENT_TOPK_WEIGHT) * top_support
                + float(current_config.TEMPORAL_CLIP_TRANSIENT_RUN_WEIGHT) * (run_mean * max(run_factor, 0.5))
            )
        elif family == "sustained":
            score = (
                float(current_config.TEMPORAL_CLIP_SUSTAINED_PEAK_WEIGHT) * peak
                + float(current_config.TEMPORAL_CLIP_SUSTAINED_TOPK_WEIGHT) * top_support
                + float(current_config.TEMPORAL_CLIP_SUSTAINED_RUN_WEIGHT) * (run_mean * run_factor)
            )
        else:
            score = (0.65 * peak) + (0.25 * top_support) + (0.10 * run_mean)
...
def build_confusion_matrix(clip_results: list[ClipResult]) -> tuple[list[str], np.ndarray]:
    ...
    labels = [
        label
        for label in sorted(set(SEMANTIC_LABEL_SET))
...
def aggregate_clip_result(
    ...
    raw_clip_scores = _compute_temporal_clip_scores(labels, raw_prob_trace, raw_prob_peak)
    raw_top3 = topk_from_scores(labels, raw_clip_scores, limit=3, exclude_labels=EVENT_FOCUS_EXCLUDED_LABELS)
    raw_predicted_label, raw_predicted_confidence = raw_top3[0]
    raw_eval_labels = tuple(
        label
        for label, _score in topk_from_scores(
            labels,
            raw_clip_scores,
            limit=max(1, raw_topk),
            exclude_labels=EVENT_FOCUS_EXCLUDED_LABELS,
        )
    )
...
def evaluate_sample_pair(
    ...
    classifier.reset_temporal_state()
    ...
    for window_index, window_start_s, window_end_s, frames_window in iterate_windows(
        ...
        labels, probabilities = classifier.predict_all(mono_window, sample_rate, frames_audio=frames_window)
        probabilities = mask_probabilities_to_label_set(
            labels,
            probabilities,
            getattr(args, "active_label_set", None),
        )
        ...
        raw_prob_trace.append(np.asarray(probabilities, dtype=np.float64).copy())
...
def main() -> None:
    ...
    args.active_label_set = set()
    if args.restrict_to_manifest_labels:
        args.active_label_set = infer_active_label_set(samples)
    if args.active_label_set:
        print(
            "[EVAL] Aktif semantic label seti: "
            f"{len(args.active_label_set)} / {len(SEMANTIC_LABEL_SET)}"
        )
    ...
    try:
        ...
        raise SystemExit(
            "Semantic classifier baslatilamadi. Secilen backend'in model / kutuphane bagimliliklarini "
